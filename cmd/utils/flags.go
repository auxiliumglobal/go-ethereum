...
var (
	...
	TestnetFlag = cli.BoolFlag{
		Name:  "testnet",
		Usage: "Auxilium test network: pre-configured proof-of-authority test network",
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	urls := params.AuxiliumMainnetBootnodes
	switch {
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	CheckExclusive(ctx, DeveloperFlag, TestnetFlag, LegacyTestnetFlag, RinkebyFlag, GoerliFlag, YoloV1Flag)
	...
	switch {
	case ctx.GlobalBool(TestnetFlag.Name) || ctx.GlobalBool(LegacyTestnetFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 88
		}
